...
public class ExcelDownloadView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model
		...
		String workbookName = (String) model.get("workbookName"); // 넘겨받은 모델(다운로드 되어질 엑셀파일의 정보)
		...
		try {
			...
			// 일반적으로 HttpServletResponse response 객체의 출력 스트림(OutputStream 이나 Writer)을 사용하여
			// HTML(아니면 다른 타입의 컨텐츠)을 작성합니다.
			// 두 가지 출력 방식 : 문자와 바이트
			...
			if (workbook != null) {
				try {
					workbook.close();
					// 엑셀파일생성에 사용했던 SXSSFWorkbook workbook 은 역할을 다했으므로 메모리 관리를 위해 close로 메모리상에서
					// 삭제하도록 한다.
				} catch (Exception e) {
					e.printStackTrace();
				}
