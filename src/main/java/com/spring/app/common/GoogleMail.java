...
public class GoogleMail {
	...
	public void sendmail_withFile(Map<String, Object> paraMap) throws Exception {
		...
		*/
		...
		// 받는 사람의 메일주소
		Address toAddr = new InternetAddress((String) paraMap.get("recipient")); // 매개변수를 통해 받아온 받는사람(수신인)의 메일주소
		msg.addRecipient(Message.RecipientType.TO, toAddr);

		if (paraMap.get("arr_attachFilename") == null) { // 첨부파일(arr_attachFilename)이 없으면 내용만 전송한다.
			// 메일 본문의 내용과 형식, 캐릭터 셋 설정
			msg.setContent((String) paraMap.get("content"), "text/html;charset=UTF-8"); // content(키값)
			...
			if (paraMap.get("arr_attachFilename") != null) {
				...
				for (String attachFilename : arr_attachFilename) {
					...
					// 다음은 탐색기에서 C:\NCS\workspace_spring_boot_17\myspring\src\main\webapp\resources\email_attach_file 폴더에 가보면 첨부판 파일을 불러온다.
	...
	public void sendmail_Reservation(String emailContent, String recipient) throws Exception {
		...
		*/
		...
		// 제목 설정
		msg.setSubject("localhost:9090/myspring/에서 방문 예약일자를 알려드립니다.");
		...
		// 받는 사람의 메일주소
		Address toAddr = new InternetAddress(recipient); // 매개변수를 통해 받아온 받는사람(수신인)의 메일주소
		msg.addRecipient(Message.RecipientType.TO, toAddr);

		// 메일 본문의 내용과 형식, 캐릭터 셋 설정
		msg.setContent("<div style='font-size:14pt;'>" + emailContent + "</div>", "text/html;charset=UTF-8"); // content(키값)
