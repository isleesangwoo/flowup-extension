...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		<%-- 컨트롤러에서 넘어온 값들 넣어주기 --%>
		$("h3#doc_title").text("부서문서함"); // 문서함 이름
		...
		<%-- 새로고침 버튼을 눌렀을 때 이벤트 --%>
		$("span#re_btn").click(e=>{
			location.href = `<%= ctxPath%>/document/deptDocumentList`;
		});
	...
	function getDocumentList() {
		...
		location.href = "<%= ctxPath%>/document/deptDocumentList?currentShowPageNo=1"
				...
				<c:if test="${not empty requestScope.deptDocumentList}">
					<c:forEach var="deptDocument" items="${requestScope.deptDocumentList}">
						<tr class="document" onclick="location.href='<%= ctxPath%>/document/documentView?documentNo=${deptDocument.documentNo}&documentType=${deptDocument.documentType}';">
							...
							<td>
								<%-- 결재양식에 따라 색 주기 --%>
								<c:if test="${deptDocument.status == 0}"><span class="p-1 in_progress">진행중</span></c:if>
								<c:if test="${deptDocument.status == 1}"><span class="p-1 approved">완료</span></c:if>
								<c:if test="${deptDocument.status == 2}"><span class="p-1 rejected">반려</span></c:if>
							</td>
						</tr>
					</c:forEach>
				</c:if>
